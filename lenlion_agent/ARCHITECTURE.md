...
| **配置集中** | 本地 `~/.hermes/` 存文件；设置 `DATABASE_URL` 后会话/配置/密钥存云端 Postgres |
...
flowchart TB
  ...
  subgraph persistence [持久化]
    ...
    PG[(云端 Postgres<br/>DATABASE_URL)]
    HOME[~/.hermes/]
  end
...
└── lenlion_agent/           # 独立 Agent 项目根（本文档所在目录）
    ├── ARCHITECTURE.md      # 本文档
    ├── CLAUDE.md            # AI 速览（本地，gitignore）
...
**未迁移模块**（见 `MIGRATION.md`）：文档站、Electron 桌面端、上游全功能 React Dashboard、上游 s6-overlay 全功能 Docker/Nix 打包、`acp_adapter/` 等。Agent 采用本机运行 + 云端 Postgres，见 `DEPLOYMENT.md`。
...
| `dashboard` | 启动 FastAPI Web 服务 + React 平台界面 |
| `gateway` | 启动/管理消息网关进程 |
| `cron` | 定时任务 CRUD |
...
Lenlion 的浏览器界面由 **React 平台 SPA + FastAPI Dashboard + tui_gateway JSON-RPC 引擎** 三部分组成。`tui_gateway` 名称来自上游 Hermes TUI 时代，在本 fork 中 **仅作为 Web 聊天后端**，不再驱动终端 UI。

### 7.1 组件职责

| 组件 | 路径 | 职责 |
|------|------|------|
| **React SPA** | `web/` | 侧栏平台 UI：概览、聊天、模型、配置、密钥、工具集、技能、MCP、网关、Cron、会话、日志 |
...
导航由 React Router hash 路由驱动；聊天与管理功能 **双通道通信**：
...
| 已移除 | 替代 |
|--------|------|
| `ui-tui/` Ink/React 终端 UI | `web/` React SPA |
...
| 路径 | 用途 |
|------|------|
| `config.yaml` | 主配置（无 `DATABASE_URL` 时；有则存 Postgres） |
| `.env` | API Key 与 `DATABASE_URL`（无 `DATABASE_URL` 时；密钥有 DB 时存 Postgres） |
| `state.db` | SQLite 会话与消息（**未设置 `DATABASE_URL` 时**） |
| `sessions/` | 部分遗留/辅助会话数据 |
| `skills/`、`plugins/` | 用户扩展（始终本地文件） |
| `cron/jobs.json` | 定时任务定义 |
| `profiles/<name>/` | 多 Profile 隔离实例 |
| `logs/` | `agent.log`、`gateway.log`、`tui_gateway_crash.log` 等 |

目录解析：`hermes_constants.get_hermes_home()`，可通过 `HERMES_HOME` 覆盖。Agent 在本机运行；**不**再使用容器卷映射 `/data`。
...
当环境变量 **`DATABASE_URL`** 存在时，运行时自动切换 Postgres 后端（**推荐**：连接云端 Platform 或托管 Postgres）：
...
- **`docker/postgres/init.sql`**：Agent 表 schema；部署时应用到**云端** Postgres（与 Platform 同库）
...
Lenlion Agent **在本机运行**，通过 `DATABASE_URL` 使用**云端 Postgres**。Platform 控制平面单独部署。详见 [DEPLOYMENT.md](./DEPLOYMENT.md)。
...
| **Editable 安装** | `pip install -e ".[cli,web,mcp,cron,postgres]"` |
| **install.sh** | `scripts/install.sh` — git clone + venv 一键安装（偏上游 Hermes 风格） |
| **Dockerfile** | 仅 CI 镜像构建冒烟（非 Agent 部署路径） |
...
本机 lenlion_agent/
    lenlion / lenlion dashboard / lenlion gateway
              │
              ├── ~/.hermes/  → 技能、日志、缓存（本地）
              └── DATABASE_URL → 云端 Postgres
                        ├── sessions / messages
                        ├── platform_config / platform_secrets
                        └── 平台控制表（lenlion_platform schema）
```

- 安装需 `postgres` extra（`psycopg[binary]`）
- 云端库需先执行 `docker/postgres/init.sql` 与 `lenlion_platform/db/init.sql`
...
| 配置兼容 | 仍使用 `~/.hermes/` 本地文件；`DATABASE_URL` 时会话/配置/密钥存云端 Postgres |
| Web 界面 | React 19 平台 SPA（侧栏 + 12 视图），替代 Ink TUI + PTY 嵌入 |
| 聊天后端 | 保留 `tui_gateway` JSON-RPC，经 `/api/ws` 暴露 |
| 持久化 | 本机文件 + 云端 Postgres（`DATABASE_URL`，推荐） |
| 已移除 | TUI、`/api/pty`、Agent Docker 部署、上游 s6 Docker/Nix 打包、Desktop App、文档站 |
| 新增 | 本机 Agent + 云端 DB（`DEPLOYMENT.md`）+ `lenlion_platform` 控制平面 |
...
| 关注点 | 文件 |
|--------|------|
| CLI 路由 | `hermes_cli/main.py` |
| CLI 解析器 | `hermes_cli/_parser.py` |
| 交互 REPL | `cli.py` |
| Dashboard 服务 | `hermes_cli/web_server.py` |
| Dashboard 启动 | `cmd_dashboard` in `main.py` |
| Web 入口 | `web/src/App.tsx` |
| 平台导航 | `web/src/stores/app.ts` |
| REST 客户端 | `web/src/lib/apiClient.ts` |
| WS 客户端 | `web/src/lib/gatewayClient.ts` |
| 聊天状态 | `web/src/stores/chat.ts` |
| WS 服务端 | `tui_gateway/ws.py` |
| JSON-RPC 引擎 | `tui_gateway/server.py` |
| Agent 门面 | `run_agent.py` |
| 对话循环 | `agent/conversation_loop.py` |
| 工具 API | `model_tools.py` |
| 工具注册 | `tools/registry.py` |
| 工具集 | `toolsets.py` |
| 网关守护 | `gateway/run.py` |
| 平台基类 | `gateway/platforms/base.py` |
| 插件管理 | `hermes_cli/plugins.py` |
| 配置 | `hermes_cli/config.py` |
| Postgres 配置存储 | `hermes_cli/config_store_postgres.py` |
| 密钥中文说明 | `hermes_cli/env_i18n.py` |
| 常量 / HOME | `hermes_constants.py` |
| 会话 DB | `hermes_state.py` |
| Postgres 会话 DB | `hermes_state_postgres.py` |
| Postgres schema | `docker/postgres/init.sql` |
| Cron 调度 | `cron/scheduler.py` |
| 本地部署 | `DEPLOYMENT.md` |
| 前端构建 | `web/vite.config.ts` |
| 打包 | `pyproject.toml` |
| 迁移范围 | `MIGRATION.md` |
...
- [DEPLOYMENT.md](./DEPLOYMENT.md) — 本机 Agent + 云端 DATABASE_URL
- [MIGRATION.md](./MIGRATION.md) — 迁移与定制记录
- [AGENTS.md](./AGENTS.md) — 开发者贡献指南
- [gateway/platforms/ADDING_A_PLATFORM.md](./gateway/platforms/ADDING_A_PLATFORM.md) — 新增消息平台
- [README.hermes-upstream.md](./README.hermes-upstream.md) — 上游完整功能文档
