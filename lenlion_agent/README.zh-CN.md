...
> **Lenlion 定制版：** CLI 命令为 `lenlion`（非 `hermes`），配置目录仍为 `~/.hermes/`。项目说明见 [README.md](./README.md)。
...
支持任意模型——[Nous Portal](https://portal.nousresearch.com)、[OpenRouter](https://openrouter.ai)（200+ 模型）、[NVIDIA NIM](https://build.nvidia.com)（Nemotron）、[小米 MiMo](https://platform.xiaomimimo.com)、[z.ai/GLM](https://z.ai)、[Kimi/Moonshot](https://platform.moonshot.ai)、[MiniMax](https://www.minimax.io)、[Hugging Face](https://huggingface.co)、OpenAI，或自定义端点。使用 `lenlion model` 即可切换——无需改代码，无锁定。
...
```bash
lenlion              # 交互式 CLI — 开始对话
lenlion model        # 选择 LLM 提供商和模型
lenlion tools        # 配置启用的工具
lenlion config set   # 设置单个配置项
lenlion gateway      # 启动消息网关（Telegram、Discord 等）
lenlion setup        # 运行完整设置向导（一次性配置所有内容）
lenlion claw migrate # 从 OpenClaw 迁移（如果来自 OpenClaw）
lenlion update       # 更新到最新版本
lenlion doctor       # 诊断问题
```
...
它会通过 OAuth 登录、把 Nous 设为推理服务商，并启用 Tool Gateway。随时用 `lenlion portal info` 查看路由状态。完整说明见 [Tool Gateway 文档](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway)。
...
Hermes 有两种入口：用 `lenlion` 启动终端 UI，或运行网关从 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与之对话。进入对话后，许多斜杠命令在两种界面中通用。

| 操作 | CLI | 消息平台 |
|------|-----|----------|
| 开始对话 | `lenlion` | 运行 `lenlion gateway setup` + `lenlion gateway start`，然后给机器人发消息 |
...
**首次安装时：** 安装向导（`lenlion setup`）会自动检测 `~/.openclaw` 并在配置开始前提供迁移选项。

**安装后任意时间：**

```bash
lenlion claw migrate              # 交互式迁移（完整预设）
lenlion claw migrate --dry-run    # 预览将要迁移的内容
lenlion claw migrate --preset user-data   # 仅迁移用户数据，不含密钥
lenlion claw migrate --overwrite  # 覆盖已有冲突
```
...
使用 `lenlion claw migrate --help` 查看所有选项，或使用 `openclaw-migration` 技能进行交互式代理引导迁移（含干运行预览）。
...
`$HERMES_HOME/lenlion-agent`（通常是 `~/.hermes/hermes-agent`）。这会匹配
`lenlion update`、托管 venv、lazy dependencies、gateway 和 docs tooling 使用的布局。
