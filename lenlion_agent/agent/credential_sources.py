...
Hermes seeds its credential pool from many places:
    ...
    manual        — user ran `lenlion auth add`
...
unify here is **removal**:
    ...
    ``lenlion auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"{env_var} until you run `lenlion auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `lenlion auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `lenlion auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `lenlion auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``lenlion auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``lenlion auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `lenlion model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``lenlion auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `lenlion auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `lenlion auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `lenlion auth add copilot` to re-enable if needed.",
    ])
