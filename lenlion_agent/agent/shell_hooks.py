...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``lenlion hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
# ---------------------------------------------------------------------------
# Introspection (used by `lenlion hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``lenlion hooks test`` and ``lenlion hooks doctor``.
    ...
    produce — otherwise scripts tested via ``lenlion hooks test`` could
