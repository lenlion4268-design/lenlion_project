...
# which becomes the dominant cost of ``lenlion`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    parsing a non-trivial config dominates ``lenlion`` cold-start time
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    hermes = metadata.get("lenlion") or metadata.get("hermes") or {}
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    hermes = metadata.get("lenlion") or metadata.get("hermes")
    if not isinstance(hermes, dict):
        return []
