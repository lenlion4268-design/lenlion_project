...
def get_provider_env(name: str) -> str:
    ...
    exported into the process environment. Falls back to a bare
    ``os.getenv`` when the config module is unavailable.
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``lenlion tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``lenlion tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``lenlion tools`` picker.
