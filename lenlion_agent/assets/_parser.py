"""
Top-level argparse construction for the lenlion CLI.
...
_EPILOGUE = """
Examples:
    lenlion                        Start interactive chat
    lenlion chat -q "Hello"        Single query mode
    lenlion --tui                  Launch the modern TUI (or set display.interface: tui)
    lenlion --cli                  Force the classic REPL (overrides display.interface: tui)
    lenlion -c                     Resume the most recent session
    lenlion -c "my project"        Resume a session by name (latest in lineage)
    lenlion --resume <session_id>  Resume a specific session by ID
    lenlion setup                  Run setup wizard
    lenlion logout                 Clear stored authentication
    lenlion auth add <provider>    Add a pooled credential
    lenlion auth list              List pooled credentials
    lenlion auth remove <p> <t>    Remove pooled credential by index, id, or label
    lenlion auth reset <provider>  Clear exhaustion status for a provider
    lenlion model                  Select default model
    lenlion fallback [list]        Show fallback provider chain
    lenlion fallback add           Add a fallback provider (same picker as `lenlion model`)
    lenlion fallback remove        Remove a fallback provider from the chain
    lenlion config                 View configuration
    lenlion config edit            Edit config in $EDITOR
    lenlion config set model gpt-4 Set a config value
    lenlion gateway                Run messaging gateway
    lenlion -s hermes-agent-dev,github-auth
    lenlion -w                     Start in isolated git worktree
    lenlion gateway install        Install gateway background service
    lenlion sessions list          List past sessions
    lenlion sessions browse        Interactive session picker
    lenlion sessions rename ID T   Rename/title a session
    lenlion logs                   View agent.log (last 50 lines)
    lenlion logs -f                Follow agent.log in real time
    lenlion logs errors            View errors.log
    lenlion logs --since 1h        Lines from the last hour
    lenlion debug share             Upload debug report for support
    lenlion update                 Update to latest version
    lenlion dashboard              Start web UI dashboard (port 9119)
    lenlion dashboard --stop       Stop running dashboard processes
    lenlion dashboard --status     List running dashboard processes

For more help on a command:
    lenlion <command> --help
"""
...
def build_top_level_parser():
    ...
    # Mirrors `lenlion chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
