...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart lenlion gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "lenlion update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `lenlion gateway restart`
    ...
    (r'\b(pkill|killall)\b.*\b(hermes|gateway|cli\.py)\b', "kill lenlion/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill lenlion` but not
    # `kill -9 $(pgrep -f lenlion)` because the substitution is opaque
