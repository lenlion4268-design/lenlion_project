"""
Backup and import commands for lenlion CLI.

`lenlion backup` creates a zip archive of the entire ~/.hermes/ directory
(excluding the hermes-agent repo and transient files).

`lenlion import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
# File names that ``lenlion import`` must never overwrite, matched by basename so
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to lenlion root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    print(f"\nRestore with: lenlion import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a lenlion home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a lenlion dir name
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: lenlion profile list  (after installing lenlion)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: lenlion update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  lenlion -p {pname} gateway install")

        print("Done. Your Hermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and lenlion backup --quick)
# ---------------------------------------------------------------------------
...
# — `lenlion update` snapshots this set before pulling so approved-user lists
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``lenlion update``.
...
def run_quick_backup(args) -> None:
    """CLI entry point for lenlion backup --quick."""
...
def create_pre_update_backup(
    ...
    (``lenlion update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `lenlion claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `lenlion import` and the
