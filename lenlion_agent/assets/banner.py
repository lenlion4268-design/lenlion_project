...
# (e.g. nix-built lenlion — no local git history to count against).
...
def check_for_updates() -> Optional[int]:
    ...
    # and `lenlion update` correctly refuses to run in-place inside the
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built lenlion; we know an update
    ...
    # Pip-install warning — `pip install lenlion-agent` is not the supported
