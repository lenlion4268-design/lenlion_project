...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("lenlion/hermes-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned lenlion/* and pr-* branches."""

    def test_prunes_orphaned_hermes_branch(self, git_repo):
        """lenlion/hermes-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "lenlion/hermes-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "lenlion/hermes-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "lenlion/hermes-deadbeef" in result.stdout
        ...
        assert "lenlion/hermes-deadbeef" in orphaned
        ...
        assert "lenlion/hermes-deadbeef" not in result.stdout
