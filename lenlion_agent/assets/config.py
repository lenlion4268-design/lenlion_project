...
- lenlion config          - Show current configuration
- lenlion config edit     - Open config in editor
- lenlion config set      - Set a specific value
- lenlion config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``lenlion config set`` (which rewrites
    ...
    ``config.yaml`` in place: lenlion never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``lenlion logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  lenlion config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``lenlion update``.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `lenlion plugins enable
    # observability/langfuse` or `lenlion tools → Langfuse`); credentials gate
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    ``docker`` and ``lenlion update`` refuses to run ("doesn't apply inside the
    ...
    #    container, and honouring it wrongly blocks ``lenlion update``.
    try:
        method = (
...
def is_uv_tool_install() -> bool:
    ...
    ``lenlion update`` to upgrade the wrong copy. It would also block on a
...
# Long-form text for ``lenlion update`` / ``--check`` when running inside the
...
✗ ``lenlion update`` doesn't apply inside the Docker container.
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``lenlion update`` inside Docker.
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the lenlion group can
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # bundled Hermes image, which drops to the `lenlion` user via
    ...
    # Opt in via ``lenlion chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: lenlion sweeps the checkpoint base at startup
    ...
    "auxiliary": {
        ...
        # Invoked by ``lenlion kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``lenlion kanban decompose`` and the kanban
        ...
        # ``lenlion profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `lenlion model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `lenlion` (and `lenlion chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `lenlion --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `lenlion -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `lenlion --tui` drops a one-time hint
    ...
    # cron daemon.
    #
    # See `lenlion curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `lenlion curator run --consolidate` overrides
        ...
        # normally restored on every `lenlion update`, so pruning them only
        ...
        # user can roll back with `lenlion curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `lenlion doctor --ack
    ...
    # todos to ready, and fires `lenlion -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `lenlion` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `lenlion kanban decompose <id>` or
    ...
    "model_catalog": {
        ...
        # next /model or `lenlion model` invocation; network failures
    ...
    "sessions": {
        ...
        # default of ``lenlion sessions prune``.
        "retention_days": 90,
    ...
    # ``lenlion update`` behaviour.
    "updates": {
        # Run a full ``lenlion backup``-style zip of HERMES_HOME before every
        # ``lenlion update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``lenlion import <path>``.  Off by default —
        ...
        "backup_keep": 5,
        # What `lenlion update` does with uncommitted local changes to the
    ...
    # `lenlion tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    "secrets": {
        "bitwarden": {
            ...
            # `lenlion secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'lenlion model' for endpoint-specific config)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @lenlion:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #lenlion)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `lenlion update`.  Skill-config prompting is a
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'lenlion setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'lenlion doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `lenlion plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `lenlion curator status` has no
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: lenlion config set <key> <value>")
...
#   openai-codex (OAuth — lenlion auth) — OpenAI Codex
#   nous         (OAuth — lenlion auth) — Nous Portal
...
def show_config():
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'lenlion doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  lenlion config edit     # Edit config file", Colors.DIM))
    print(color("  lenlion config set <key> <value>", Colors.DIM))
    print(color("  lenlion setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: lenlion config set <key> <value>")
            print()
            print("Examples:")
            print("  lenlion config set model anthropic/claude-sonnet-4")
            print("  lenlion config set terminal.backend docker")
            print("  lenlion config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'lenlion config migrate' to add them")
        ...
        print("  lenlion config           Show current configuration")
        print("  lenlion config edit      Open config in editor")
        print("  lenlion config set <key> <value>   Set a config value")
        print("  lenlion config check     Check for missing/outdated config")
        print("  lenlion config migrate   Update config with new options")
        print("  lenlion config path      Show config file path")
        print("  lenlion config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``lenlion config`` UI so users
