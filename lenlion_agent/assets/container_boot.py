...
def reconcile_profile_gateways(
    *,
    ...
    is what ``lenlion gateway start`` (no ``-p``) targets. Without it,
    bare ``lenlion gateway start`` inside the container would land on
    ...
    # ``lenlion gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `lenlion profile create` (config.yaml
            # is not — that comes later via `lenlion setup`). Use it as the
            ...
            # via ``lenlion -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the lenlion args.

    The container PID 1 argv looks like
    ``/init /opt/hermes/docker/main-wrapper.sh <subcommand> [args...]`` and
    the wrapper re-execs ``lenlion <subcommand>``. Peel ``init`` →
    ``main-wrapper.sh`` → ``lenlion`` so callers can match on the bare
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``lenlion dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `lenlion -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit lenlion
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the lenlion user) won't hit EACCES. See
