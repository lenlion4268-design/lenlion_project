"""
Doctor command for lenlion CLI.
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``lenlion --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            "Re-sync version files (e.g. run 'lenlion update', or set "
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `lenlion profile create <name>`")
        return
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `lenlion`.
    os.environ.setdefault("HERMES_INTERACTIVE", "1")

    # Handle `lenlion doctor --ack <id>` as a fast path. Persist the ack and
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`lenlion doctor --ack {hit.advisory.id}`."
                )
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'lenlion setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'lenlion setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'lenlion setup' to create one")
                issues.append("Run 'lenlion setup' to create .env")
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'lenlion config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'lenlion setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'lenlion setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'lenlion config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'lenlion setup' to migrate config")
                else:
                    issues.append("Run 'lenlion doctor --fix' or 'lenlion setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'lenlion doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `lenlion setup` run)",
                )
                if should_fix:
        ...
        try:
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'lenlion doctor --fix' "
                        "(or 'lenlion sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'lenlion doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        if _venv_bin is None:
            check_warn(
                ...
                "(lenlion not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/hermes — run 'lenlion doctor --fix'")
                ...
                check_fail(
                    ...
                    "(lenlion command may not work outside the venv)"
                )
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hermes symlink — run 'lenlion doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `lenlion doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'lenlion config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``lenlion doctor``.
    ...
    # never the legitimate source for `lenlion doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'lenlion setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: lenlion skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: lenlion memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: lenlion memory setup",
                    "No Honcho API key — run 'lenlion memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run lenlion memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: lenlion memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: lenlion memory setup")
        except Exception as _e:
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "lenlion -p" in content:
                            _m = _re.search(r"lenlion -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'lenlion doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
