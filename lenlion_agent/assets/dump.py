...
def _redact(value: str) -> str:
    ...
    ``lenlion dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `lenlion auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `lenlion auth list` shows it (#42130).
