...
Let the lenlion agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where lenlion runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `lenlion meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lenlion meet` CLI |
...
| `cli.py` | `lenlion meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `lenlion meet node {run,list,approve,remove,status,ping}` |
| `SKILL.md` | agent usage guide |

## Local quick start

```bash
lenlion plugins enable google_meet
lenlion meet install                                      # pip + Chromium
lenlion meet setup                                        # preflight
lenlion meet auth                                         # optional
lenlion meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lenlion meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
lenlion meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
lenlion meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lenlion meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
lenlion meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, lenlion will **not** switch your system audio input automatically — the
...
lenlion plugins enable google_meet
lenlion meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
lenlion meet node approve my-mac ws://<mac-ip>:18789 <token>
lenlion meet node ping my-mac
