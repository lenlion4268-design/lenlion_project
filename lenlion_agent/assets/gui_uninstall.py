...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``lenlion desktop`` / ``lenlion gui``)
...
``lenlion uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``lenlion desktop`` inside the checkout.
