...
When HERMES_HOME is set to the lenlion root (e.g. systemd hardcodes
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    Verifies that HERMES_HOME pointing to the lenlion root does NOT suppress
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        Bug scenario from #22502: systemd sets HERMES_HOME to the lenlion root
        and the user switches to a profile via `lenlion profile use`.
    ...
    def test_subcommand_profile_flag_is_not_consumed(self, tmp_path, monkeypatch):
        ...
        When that argv is passed through `lenlion mcp add --args`, the early
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``lenlion gateway run`` (no ``-p``) to mean "the root HERMES_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        gateway is launched as bare ``lenlion gateway run`` with
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `lenlion gateway run` still honors
