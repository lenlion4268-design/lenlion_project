...
subparser, placing the flag BEFORE the subcommand (e.g. 'lenlion --yolo chat')
...
def _build_parser():
    """Build the lenlion argument parser from the real code.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `lenlion gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `lenlion <argv>` must exit 0 (help) rather than
