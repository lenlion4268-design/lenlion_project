...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "lenlion auth add xai-oauth" in body
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``lenlion model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``lenlion model``
    ...
    assert "lenlion model" not in rendered.lower()
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `lenlion model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `lenlion auth list` reflects truth and
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`lenlion auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `lenlion auth remove`.
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``lenlion auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `lenlion auth remove xai-oauth 1`.
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``lenlion model`` -> xAI Grok OAuth (or another process
    ...
    # Simulate the user re-running `lenlion model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``lenlion auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`lenlion auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``lenlion auth status``, ``lenlion setup``, and the
    ``lenlion`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
