"""Tests for ``lenlion debug`` CLI command and debug utilities."""
...
class TestRunDebugShare:
    ...
    def test_share_uploads_four_pastes(self, hermes_home, capsys):
        ...
        with patch("hermes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- lenlion dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``lenlion debug``
    invocation.
    """
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_survives_sweep_failure(self, hermes_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: lenlion debug" in out
...
class TestBuildDebugShare:
    ...
    Backs both ``lenlion debug share`` (CLI) and ``POST /api/ops/debug-share``
