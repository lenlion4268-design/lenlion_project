...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "lenlion --profile alice gateway start" in out
...
# resume_pending=True) never fires when `lenlion gateway stop` kills the
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `lenlion gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
