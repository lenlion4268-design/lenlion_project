"""Tests for the multi-board kanban layer (``lenlion kanban boards …``).
...
* CLI surface: ``lenlion kanban boards list/create/switch/rm``.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``lenlion kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
