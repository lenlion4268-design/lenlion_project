"""Regression test: ``lenlion mcp add --command`` must not clobber the
...
result, ``lenlion mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the lenlion parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`lenlion mcp add foo --url ...` must keep args.command == "mcp".
