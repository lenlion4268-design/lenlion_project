"""Tests for the ``lenlion send`` CLI subcommand.
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``lenlion send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``lenlion config set``) lives
