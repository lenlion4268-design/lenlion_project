"""Guard: every `lenlion update` path that reports user-modified skills must
also tell the user how to find them.

`lenlion update` keeps (does not overwrite) bundled skills the user edited and
...
at ``lenlion skills list-modified`` so the count is actionable — otherwise,
...
_HINT_RE = re.compile(r"lenlion skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`lenlion skills list-modified` within the following lines — the "
