...
base deps) during ``lenlion update``, leaving the venv in a broken state
...
class TestVerifyCoreDependencies:
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        is left missing and ``lenlion`` drops off PATH. Previously this path
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `lenlion update`.
