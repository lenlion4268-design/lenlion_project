...
This means ``lenlion dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``lenlion kanban
...
def list_diagnostics(
    ...
    AND for ``lenlion kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``lenlion kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``lenlion kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``lenlion kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``lenlion profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``lenlion kanban decompose <task_id>``.
