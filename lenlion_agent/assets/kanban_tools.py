...
orchestrator work. A normal ``lenlion chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``lenlion kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``lenlion kanban complete …``
   inside the container, where ``lenlion`` isn't installed and the DB
...
Humans continue to use the CLI (``lenlion kanban …``), the dashboard
(``lenlion dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``lenlion chat`` without the kanban toolset see zero
