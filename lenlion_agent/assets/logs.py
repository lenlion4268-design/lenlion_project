"""``lenlion logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    lenlion logs                    # last 50 lines of agent.log
    lenlion logs -f                 # follow agent.log in real time
    lenlion logs errors             # last 50 lines of errors.log
    lenlion logs gateway -n 100    # last 100 lines of gateway.log
    lenlion logs gui -f            # follow gui.log (dashboard/pty/ws)
    lenlion logs desktop -f        # follow desktop.log (Electron app boot/backend)
    lenlion logs --level WARNING    # only WARNING+ lines
    lenlion logs --session abc123   # filter by session ID substring
    lenlion logs --component tools  # only tool-related lines
    lenlion logs --since 1h         # lines from the last hour
    lenlion logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Hermes runs — try 'lenlion chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'lenlion chat' to generate logs)")
