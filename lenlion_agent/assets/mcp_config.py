"""
MCP Server Management CLI — ``lenlion mcp`` subcommand.

Implements ``lenlion mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``lenlion mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  lenlion mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  lenlion mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  lenlion mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: lenlion mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `lenlion mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  lenlion mcp add <name> --url <endpoint>')
        _info('  lenlion mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `lenlion mcp remove` + `lenlion mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `lenlion mcp login " + name + "`.")
            return
        if tools:
...
# ─── lenlion mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'lenlion mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``lenlion mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `lenlion plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("lenlion mcp                                    Open the catalog picker (default)")
        _info("lenlion mcp catalog                            List Nous-approved MCPs")
        _info("lenlion mcp install <name>                     Install a catalog MCP")
        _info("lenlion mcp serve                              Run as MCP server")
        _info("lenlion mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("lenlion mcp add <name> --command <cmd>         Add a stdio server")
        _info("lenlion mcp add <name> --preset <preset>       Add from a known preset")
        _info("lenlion mcp remove <name>                      Remove a server")
        _info("lenlion mcp list                               List configured servers")
        _info("lenlion mcp test <name>                        Test connection")
        _info("lenlion mcp configure <name>                   Toggle tools")
        _info("lenlion mcp login <name>                       Re-authenticate OAuth")
        print()
