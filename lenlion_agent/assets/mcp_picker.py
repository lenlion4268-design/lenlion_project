"""MCP picker — interactive `lenlion mcp picker` (also the default `lenlion mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``lenlion mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `lenlion plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `lenlion mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: lenlion mcp install <name>    Picker: lenlion mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `lenlion update` "
...
def show_catalog() -> None:
    """`lenlion mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`lenlion mcp picker` (and default `lenlion mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`lenlion mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `lenlion mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
