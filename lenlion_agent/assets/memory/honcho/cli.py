...
Handles: lenlion honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'lenlion honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'lenlion honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `lenlion update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'lenlion honcho setup' again once you have a key.\n")
            return
    ...
    try:
        ...
        print("  Run: lenlion config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    lenlion honcho status     -- show full config")
    print("    lenlion honcho mode       -- change recall/observation mode")
    print("    lenlion honcho tokens     -- tune context and dialectic budgets")
    print("    lenlion honcho peer       -- update peer names")
    print("    lenlion honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: lenlion honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'lenlion honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'lenlion honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: lenlion honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Hermes' identity in Honcho. Seed with 'lenlion honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: lenlion honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: lenlion honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("\n  Set with: lenlion honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'lenlion honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'lenlion honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    lenlion honcho identity --show        — show both peer representations")
        print("    lenlion honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  lenlion honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'lenlion honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'lenlion honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  lenlion honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        for f in user_files:
            print("    lenlion honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'lenlion honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'lenlion honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    lenlion honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  lenlion honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  lenlion honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. lenlion honcho setup              — configure API key (required)")
        print("  2. lenlion honcho migrate            — re-run this walkthrough")
    else:
        print("  1. lenlion honcho status             — verify Honcho connection")
        print("  2. lenlion                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. lenlion honcho identity --show    — verify AI peer representation")
        print("  4. lenlion honcho tokens             — tune context and dialectic budgets")
        print("  5. lenlion honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'lenlion memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``lenlion honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``lenlion honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to lenlion memory setup)",
    )
