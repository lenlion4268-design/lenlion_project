...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``lenlion config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'lenlion model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'lenlion doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Build curated model lists keyed by lenlion provider ID
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # /model picker sees the SAME list `lenlion model` would build, with
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `lenlion model` picker deliberately shows ONLY the curated agentic
