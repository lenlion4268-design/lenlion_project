...
lenlion photon setup --phone +15551234567

# Start the gateway
lenlion gateway start
```

`lenlion photon setup` does, in order:
...
Run `lenlion photon status` to see what's configured.
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `lenlion photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `lenlion photon status`, a DM and a group roundtrip,
