...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a lenlion dependency
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `lenlion photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `lenlion photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `lenlion photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `lenlion photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `lenlion photon setup`)"
