"""CLI handlers for the ``lenlion proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "lenlion proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"lenlion auth add {adapter.name}")
        print(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print(
        "\nStart the proxy with: lenlion proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``lenlion proxy <subcommand>``."""
    ...
    print(
        "lenlion proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  lenlion proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  lenlion proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  lenlion proxy providers\n"
