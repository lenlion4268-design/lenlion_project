...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``lenlion`` invocations don't
  hammer the API.
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `lenlion chat -q ...`
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``lenlion secrets bitwarden setup``
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `lenlion secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `lenlion secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `lenlion secrets bitwarden setup` to install."
        )
        return result
