"""CLI handlers for ``lenlion secrets bitwarden ...``.
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]lenlion secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]lenlion secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]lenlion secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]lenlion secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`lenlion secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]lenlion secrets bitwarden setup[/cyan] and pick EU or "
