"""CLI subcommand: ``lenlion send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"lenlion send: {file_path} is not a text file. --file reads the "
                ...
                f'  lenlion send --to telegram "MEDIA:{file_path}"\n'
                f'  lenlion send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  lenlion send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"lenlion send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"lenlion send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"lenlion send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"lenlion send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"lenlion send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `lenlion gateway setup`, or run the gateway once so")
...
def _load_hermes_env() -> None:
    ...
    startup that ``lenlion send`` must replicate when invoked standalone:
    ...
    2. Bridge top-level simple values from ``~/.hermes/config.yaml`` into
       ...
       via ``lenlion config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``lenlion send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "lenlion send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  lenlion send --to telegram \"hello\"\n"
            "  lenlion send --to discord:#ops --file report.md\n"
            "  lenlion send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "lenlion send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `lenlion send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  lenlion send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | lenlion send --to telegram:-1001234567890\n"
            "  lenlion send --to discord:#ops --file /tmp/report.md\n"
            "  lenlion send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  lenlion send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  lenlion send --list                  # all platforms\n"
            "  lenlion send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `lenlion send --list telegram`.",
    )
