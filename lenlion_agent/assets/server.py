...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `lenlion --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `lenlion --tui`, so it must NOT count as dead here (doing
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `lenlion` shows the prompt before constructing AIAgent; the TUI used
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # The desktop app and `lenlion --tui` both land here. See
...
def _(rid, params: dict) -> dict:
    ...
    # ``lenlion --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``lenlion gateway`` process, whose
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `lenlion` is interactive — use `/hermes chat -q …` or run `lenlion` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`lenlion setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`lenlion gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`lenlion sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`lenlion config edit` needs $EDITOR in a real terminal"
    return None
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `lenlion model` to configure",
            )
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``lenlion plugins`` / the dashboard, so the three surfaces
