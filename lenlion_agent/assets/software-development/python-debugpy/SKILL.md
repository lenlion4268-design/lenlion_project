...
- A long-running process (lenlion gateway, tui_gateway) misbehaves and you can't restart it
...
The lenlion test runner and pytest both support this:
...
Easiest: add `breakpoint()` near the suspect line, then run `lenlion` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `lenlion --tui`)
...
debugpy.wait_for_client()
```
Start `lenlion --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
