...
def configure_windows_stdio() -> bool:
    ...
    # already-running shells don't see the change — which means lenlion
...
def _default_windows_editor() -> str:
    ...
    ``lenlion config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``lenlion`` from right after install) retain their old PATH.
    ...
    candidate_dirs = [
        ...
        # on PATH when the user invokes lenlion, but harmless to include.
