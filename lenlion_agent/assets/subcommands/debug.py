"""``lenlion debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hermes Agent. Use 'lenlion debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    lenlion debug share              Upload debug report and print URL
    lenlion debug share --lines 500  Include more log lines
    lenlion debug share --expire 30  Keep paste for 30 days
    lenlion debug share --local      Print report locally (no upload)
    lenlion debug share --no-redact  Disable upload-time secret redaction
    lenlion debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'lenlion debug share'",
    )
