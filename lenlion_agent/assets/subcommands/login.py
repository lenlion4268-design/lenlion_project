"""``lenlion login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``lenlion login`` was removed in favor of ``lenlion auth`` / ``lenlion model``
    ...
    old scripts/aliases invoking ``lenlion login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``lenlion --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``lenlion login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``lenlion model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `lenlion auth` to manage credentials, "
            "`lenlion model` to select a provider, or `lenlion setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `lenlion model`",
    )
    login_parser.add_argument(
