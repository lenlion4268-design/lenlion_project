...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or lenlion -c.",
    ...
    # --- CLI Flags ---
    "lenlion -c resumes your most recent CLI session. lenlion -c \"project name\" resumes by title.",
    "lenlion -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "lenlion -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "lenlion chat -t web,terminal enables only specific toolsets for a focused session.",
    "lenlion chat -s github-pr-workflow preloads a skill at launch.",
    "lenlion chat -q \"query\" runs a single non-interactive query and exits.",
    "lenlion chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "lenlion chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "lenlion --yolo bypasses all dangerous command approval prompts for the entire session.",
    "lenlion chat --source telegram tags the session for filtering in lenlion sessions list.",
    "lenlion -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "lenlion doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "lenlion dump outputs a compact setup summary — great for bug reports.",
    "lenlion config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "lenlion config edit opens config.yaml in your default editor.",
    "lenlion config check scans for missing or stale configuration options.",
    "lenlion sessions browse opens an interactive session picker with search.",
    "lenlion sessions stats shows session counts by platform and database size.",
    "lenlion sessions prune --older-than 30 cleans up old sessions.",
    "lenlion skills search react --source skills-sh searches the skills.sh public directory.",
    "lenlion skills check scans installed hub skills for upstream updates.",
    "lenlion skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "lenlion skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "lenlion mcp add github --command npx adds MCP servers from the command line.",
    "lenlion mcp serve runs Hermes itself as an MCP server for other agents.",
    "lenlion auth add lets you add multiple API keys for credential pool rotation.",
    "lenlion completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "lenlion logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "lenlion backup creates a zip backup of your entire Hermes home directory.",
    "lenlion profile create coder creates an isolated profile that becomes its own command.",
    "lenlion profile create work --clone copies your current config and keys to a new profile.",
    "lenlion update syncs new bundled skills to ALL profiles automatically.",
    "lenlion gateway install sets up Hermes as a system service (systemd/launchd).",
    "lenlion memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "lenlion webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: lenlion tools disables unused tools, lenlion skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "lenlion models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as lenlion -w).",
    ...
    "Profile names become shell commands — 'lenlion profile create coder' creates the 'coder' command.",
    "lenlion profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "lenlion sessions export backup.jsonl exports all sessions for backup or analysis.",
    "lenlion -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "lenlion skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: lenlion cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "lenlion gateway install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    "lenlion mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "lenlion mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "lenlion mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — lenlion tools can toggle them per platform.",
    ...
    "lenlion chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "lenlion plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "lenlion acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "lenlion auth add nous or lenlion auth add openai-codex sets up OAuth-based providers.",
    ...
    "lenlion status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'lenlion profile create ls' would be rejected.",
    "lenlion profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "lenlion claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via lenlion tools.",
    ...
    # --- Curator ---
    'lenlion curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "lenlion curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'lenlion curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'lenlion auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'lenlion dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'lenlion dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'lenlion -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'lenlion chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'lenlion chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'lenlion chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "lenlion chat --source tool tags programmatic chats so they don't clutter lenlion sessions list.",
    'lenlion dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'lenlion sessions rename <ID> "new title" renames any past session; lenlion sessions delete <ID> removes one.',
    'lenlion import restores a session export or profile archive produced by sessions export or profile export.',
    'lenlion fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'lenlion pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'lenlion setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'lenlion status --deep runs the full health sweep across every component; plain lenlion status is the quick view.',
    ...
    '--source tool chats are excluded from lenlion sessions list by default — set --source explicitly to see them.',
