...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, lenlion accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, lenlion accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into lenlion,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, lenlion still runs as PID 1 and zombies "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    # actually carries the package source so ``await import('@lenlion/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@lenlion/ink``)
    ...
    assert "ui-tui/packages/hermes-ink/" in dockerfile_text, (
        ...
        "so ``await import('@lenlion/ink')`` resolves at runtime."
    )
