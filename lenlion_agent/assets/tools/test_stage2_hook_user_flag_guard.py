...
root, and the baked image dirs are owned by the lenlion build UID, so an
...
PUID/PGID aliases), which remaps the lenlion user and chowns the volume.

The guard fires only when the current UID is neither root NOR the lenlion UID,
so the #34648 `--user 10000:10000` case (pinning to the lenlion UID itself) is
unaffected.
...
def test_guard_present_and_mentions_remediation(path: Path) -> None:
    ...
    assert '"$cur_uid" != "$(id -u lenlion)"' in block
...
def _run_guard(text: str, *, cur_uid: int, hermes_uid: int = 10000) -> subprocess.CompletedProcess:
    ...
    with tempfile.TemporaryDirectory() as d:
        script = (
            "set -e\n"
            # Stub `id`: `id -u` -> cur_uid; `id -u lenlion` -> hermes_uid.
            f'id() {{ if [ "$2" = lenlion ]; then echo {hermes_uid}; else echo {cur_uid}; fi; }}\n'
            + block
...
def test_arbitrary_user_uid_is_rejected() -> None:
    """An arbitrary host UID (1000), neither root nor lenlion, is rejected."""
...
def test_user_pinned_to_hermes_uid_passes() -> None:
    """`--user 10000:10000` (the lenlion UID itself) is the supported non-root
...
def test_user_pinned_to_remapped_hermes_uid_passes() -> None:
    """After a HERMES_UID remap the lenlion UID is e.g. 4242; a container pinned
