...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing hermes-agent or lenlion PATH entries
            new_lines = []
            skip_next = False
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing lenlion
...
def remove_wrapper_script():
    """Remove the lenlion wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "hermes",
        Path("/usr/local/bin/lenlion"),
    ]
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``lenlion`` command:
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``lenlion gateway run`` processes
...
#   4. On the ``lenlion dashboard`` + gateway paths, drops files into
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``lenlion -p <name> gateway stop|uninstall`` because
    ...
    #    Use `python -m hermes_cli.main` so we don't depend on a `lenlion`
...
def run_gui_uninstall(args):
    ...
    Mirrors ``lenlion uninstall --gui``. Removes the desktop app's built
    ...
    print("or 'lenlion uninstall' to remove the agent too.")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``lenlion`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    ...
    log_info("Removing lenlion command...")
