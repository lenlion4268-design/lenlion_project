...
Backend is selected during ``lenlion tools`` setup (web.backend in config.yaml).
...
def _env_value(name: str) -> str:
    ...
    through Hermes' config/.env layer (``lenlion config set``, ``lenlion tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``lenlion tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `lenlion tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `lenlion tools` to set one up."
                ),
            }
        else:
            logger.info(
