"""lenlion webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    lenlion webhook subscribe <name> [options]
...
def _setup_hint() -> str:
  ...
  Then start the gateway: lenlion gateway run
"""
...
def webhook_command(args):
    """Entry point for 'lenlion webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: lenlion webhook {subscribe|list|remove|test}")
        print("Run 'lenlion webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (lenlion gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: lenlion webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from lenlion webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (lenlion gateway run)")
