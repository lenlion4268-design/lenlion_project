...
def load_cli_config() -> Dict[str, Any]:
    ...
    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``lenlion chat --ignore-user-config``),
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && lenlion` to control it.
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``lenlion --help`` paths inside the chat command flow).  See
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run lenlion -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"hermes-{short_id}"
    branch_name = f"lenlion/{wt_name}"
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``lenlion/*`` and ``pr-*`` local branches that
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``lenlion/hermes-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``lenlion -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("lenlion/hermes-") or b.startswith("pr-"))
    ]
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # by `lenlion chat --ignore-rules` in hermes_cli/main.py. When true we
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``lenlion doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``lenlion doctor`` so the banner stays
        small.
        """
        try:
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'lenlion setup' to configure[/]")
        except Exception:
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``lenlion sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `lenlion sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``lenlion --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: lenlion --resume {self.session_id}")
        except Exception as e:
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `lenlion model` and gateway pickers).
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                # Discover from disk (bundled + user), matching `lenlion plugins
                ...
                if not user_entries:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: lenlion plugins list)")
                    print("  Enable/disable: lenlion plugins enable/disable <name>")
            except Exception as e:
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `lenlion portal` to log in, then /credits.")
            return
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  lenlion --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  lenlion -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the lenlion process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: lenlion setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `lenlion sessions list`
...
def main(
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``lenlion chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                # Single-query mode (`lenlion chat -q "…"`): skip the welcome
