...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``lenlion://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``lenlion://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"lenlion://blueprint/{quote(blueprint.key)}{qs}"
