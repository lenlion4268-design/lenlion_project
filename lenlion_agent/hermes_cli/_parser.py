...
Examples:
    ...
    {CLI_COMMAND} setup                  Run setup wizard
    {CLI_COMMAND} logout                 Clear stored authentication
    {CLI_COMMAND} auth add <provider>    Add a pooled credential
    {CLI_COMMAND} auth list              List pooled credentials
    {CLI_COMMAND} auth remove <p> <t>    Remove pooled credential by index, id, or label
    {CLI_COMMAND} auth reset <provider>  Clear exhaustion status for a provider
    {CLI_COMMAND} model                  Select default model
    {CLI_COMMAND} fallback [list]        Show fallback provider chain
    {CLI_COMMAND} fallback add           Add a fallback provider (same picker as `{CLI_COMMAND} model`)
    {CLI_COMMAND} fallback remove        Remove a fallback provider from the chain
    {CLI_COMMAND} config                 View configuration
    {CLI_COMMAND} config edit            Edit config in $EDITOR
    {CLI_COMMAND} config set model gpt-4 Set a config value
    {CLI_COMMAND} gateway                Run messaging gateway
    {CLI_COMMAND} -s hermes-agent-dev,github-auth
    {CLI_COMMAND} -w                     Start in isolated git worktree
    {CLI_COMMAND} gateway install        Install gateway background service
    {CLI_COMMAND} sessions list          List past sessions
    {CLI_COMMAND} sessions browse        Interactive session picker
    {CLI_COMMAND} sessions rename ID T   Rename/title a session
    {CLI_COMMAND} logs                   View agent.log (last 50 lines)
    {CLI_COMMAND} logs -f                Follow agent.log in real time
    {CLI_COMMAND} logs errors            View errors.log
    {CLI_COMMAND} logs --since 1h        Lines from the last hour
    {CLI_COMMAND} debug share             Upload debug report for support
    {CLI_COMMAND} update                 Update to latest version
    {CLI_COMMAND} dashboard              Start web UI dashboard (port 9119)
    {CLI_COMMAND} dashboard --stop       Stop running dashboard processes
    {CLI_COMMAND} dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog=CLI_COMMAND,
        description="Lenlion Agent - AI assistant with tool-calling capabilities",
