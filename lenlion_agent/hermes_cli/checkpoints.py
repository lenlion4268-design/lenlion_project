"""`lenlion checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hermes/checkpoints/``.  Actions:

    lenlion checkpoints               # same as `status`
    lenlion checkpoints status        # total size, project count, breakdown
    lenlion checkpoints list          # per-project checkpoint counts + workdir
    lenlion checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    lenlion checkpoints clear [-f]    # nuke the entire base (asks first)
    lenlion checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    lenlion checkpoints
    lenlion checkpoints prune --retention-days 3 --max-size-mb 200
    lenlion checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lenlion checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lenlion checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lenlion checkpoints` → status
