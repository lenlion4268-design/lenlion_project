"""
Cron subcommand for lenlion CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"((?:lenlion|hermes)\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*(?:lenlion|hermes))"
    r"|(systemctl\s+(restart|stop|start)\s+.*(?:lenlion|hermes))"
    r"|(p?kill\s+.*(?:lenlion|hermes).*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'lenlion cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: lenlion gateway install", Colors.DIM))
        print(color("                    sudo lenlion gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    lenlion gateway install    # Install as a user service")
        print("    sudo lenlion gateway install --system  # Linux servers: boot-time system service")
        print("    lenlion gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `lenlion gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: lenlion cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
