"""``lenlion debug`` debug tools for Hermes Agent.

Currently supports:
    lenlion debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[lenlion debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``lenlion debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``lenlion debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `lenlion debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``lenlion debug share`` invocation added ~20 MB of resident Python
    ...
    ``lenlion debug share`` also runs an opportunistic sweep as a fallback
...
def _capture_dump() -> str:
    """Run ``lenlion dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``lenlion dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``lenlion debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "lenlion debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `lenlion debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  lenlion debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: lenlion debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'lenlion debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``lenlion debug`` call.
    ...
    # best-effort — any failure is swallowed so ``lenlion debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: lenlion debug <command>")
        print()
        print("Commands:")
