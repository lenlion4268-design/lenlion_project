"""
lenlion fallback — manage the fallback provider chain.
...
Subcommands:
  lenlion fallback [list]   Show the current fallback chain (default when no subcommand)
  lenlion fallback add      Pick provider + model via the same picker as `lenlion model`,
                           then append the selection to the chain
  lenlion fallback remove   Pick an entry to delete from the chain
  lenlion fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `lenlion model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  lenlion fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `lenlion model`, then append the selection to the chain."""
    ...
    print("  `lenlion model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `lenlion fallback list` to view, or `lenlion fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``lenlion fallback [subcommand]``."""
