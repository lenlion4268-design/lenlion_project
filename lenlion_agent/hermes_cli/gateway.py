"""
Gateway subcommand for lenlion CLI.

Handles: lenlion gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `lenlion update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "lenlion CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``lenlion gateway status`` from falsely counting the ``lenlion`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``lenlion gateway status``) is never mistaken for a running
    # gateway.  See #13242.
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``lenlion update``
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``lenlion update`` users
    # with no running gateway until they re-invoke ``lenlion gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}lenlion gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}lenlion gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `lenlion gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``lenlion gateway status`` output so users with
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``lenlion gateway run`` must remain interruptible from
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("    lenlion gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: lenlion gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo lenlion gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    lenlion gateway uninstall")
    print_info("    sudo lenlion gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo lenlion gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo lenlion gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo lenlion gateway start --system")
            return scope, False
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``lenlion update`` later relocates/removes — the path
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``lenlion gateway restart``
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo lenlion gateway uninstall --system")
    print_info("         lenlion gateway install")
    print_info("         lenlion gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}lenlion gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}lenlion gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}lenlion gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `lenlion gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `lenlion gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}lenlion gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}lenlion gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}lenlion gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}lenlion gateway start{scope_flag}"
        )
...
# process invocation (each ``lenlion gateway start/stop/status`` call).
...
# (the documented `nohup lenlion gateway run` workaround). See #23387.
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup lenlion gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: lenlion gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup lenlion gateway run --replace "
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  lenlion gateway status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `lenlion gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: lenlion gateway start")
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``lenlion gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``lenlion gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'lenlion gateway restart' to replace it,")
    print("  or 'lenlion gateway stop' first.")
    print("  Or use 'lenlion gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `lenlion gateway run` still
    ...
    # /restart) rather than through `lenlion gateway restart` which already
    ...
    # `lenlion gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @lenlion:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. lenlion) and copy the bot token",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: lenlion pairing generate bluebubbles",
...
def _all_platforms() -> list[dict]:
    ...
    ``lenlion setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run lenlion under
        WSL.
    """
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'lenlion pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: lenlion pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'lenlion gateway setup'"
                    )
            continue
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: lenlion pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'lenlion gateway setup'")
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `lenlion gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `lenlion pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `lenlion pairing approve`."
        )
    elif access_idx == 1:
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: lenlion gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: lenlion gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: lenlion gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo lenlion gateway install --system"
                        )
                    print_info("  Or run in foreground:  lenlion gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: lenlion gateway run")
                print_info(
                    "  For persistence:   tmux new -s lenlion 'lenlion gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: lenlion gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup lenlion gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'lenlion gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``lenlion gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``lenlion gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: lenlion gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: lenlion gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s lenlion 'lenlion gateway run'"
                )
                print()
            ...
            print()
            print(
                "  lenlion gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s lenlion 'lenlion gateway run'         # persistent via tmux"
            )
            print(
                "  nohup lenlion gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  lenlion profile create <name>     # creates the s6 service slot")
                print("  lenlion -p <name> gateway start   # bring it up via s6")
                print("  lenlion status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: lenlion gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: lenlion gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: lenlion gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  lenlion profile delete <name>     # tears down the s6 service slot")
                print("  lenlion -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `lenlion -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: lenlion gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `lenlion gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `lenlion gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: lenlion gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  lenlion gateway install")
                else:
                    print("To install as a service:")
                    print("  lenlion gateway install")
                    print("  sudo lenlion gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  lenlion gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup lenlion gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s lenlion 'lenlion gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  lenlion gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  lenlion gateway install  # Install as user service")
                    print(
                        "  sudo lenlion gateway install --system  # Install as boot-time system service"
                    )
