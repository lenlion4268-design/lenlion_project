"""lenlion hooks — inspect and manage shell-script hooks.

Usage::

    lenlion hooks list
    lenlion hooks test <event> [--for-tool X] [--payload-file F]
    lenlion hooks revoke <command>
    lenlion hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``lenlion hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lenlion hooks {list|test|revoke|doctor}")
        print("Run 'lenlion hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        ...
        print("See `lenlion hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lenlion hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lenlion hooks test` and `lenlion hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lenlion hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lenlion hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lenlion hooks doctor`.")
