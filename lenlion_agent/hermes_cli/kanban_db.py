...
``lenlion -p <profile>`` joins the same board as the dispatcher that
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``lenlion kanban boards
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``lenlion kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``lenlion kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``lenlion kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``lenlion kanban log`` unambiguous even when multiple
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``lenlion kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``lenlion --skills ...`` alongside the built-in
    ...
    # `lenlion --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``lenlion kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `lenlion kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``lenlion kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``lenlion kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``lenlion kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``lenlion kanban complete <id>``) works without requiring
...
#      via ``lenlion kanban show <id>`` and the dashboard.
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``lenlion kanban
...
def dispatch_once(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``lenlion -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `lenlion kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    ...
    # pyproject.toml, NOT a top-level ``lenlion`` package — there is no
    # ``lenlion`` package to import.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``lenlion`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$LENLION_BIN`` / ``$HERMES_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("lenlion")`` — the console-script shim, normalized to
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Hermes is launched from a venv and the ``lenlion`` shim is not on
    ...
    cli_bin = _safe_which_no_cwd(CLI_COMMAND) if _IS_WINDOWS else shutil.which(CLI_COMMAND)
    if cli_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``lenlion -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``lenlion -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `lenlion -p <name>` the
    ...
    # that even when the worker activates a profile (`lenlion -p <name>`
    ...
    # `lenlion -p <assignee>` activates the profile, but the env var is
    ...
    # `lenlion kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`lenlion` executable not found on PATH. "
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``lenlion kanban daemon`` is systemd-friendly.
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``lenlion kanban assignees`` for the terminal.
