...
Usage:
    lenlion                     # Interactive chat (default)
    lenlion chat                # Interactive chat
    lenlion gateway             # Run gateway in foreground
    lenlion gateway start       # Start gateway as service
    lenlion gateway stop        # Stop gateway service
    lenlion gateway status      # Show gateway status
    lenlion gateway install     # Install gateway service
    lenlion gateway uninstall   # Uninstall gateway service
    lenlion setup               # Interactive setup wizard
    lenlion logout              # Clear stored authentication
    lenlion status              # Show status of all components
    lenlion cron                # Manage cron jobs
    lenlion cron list           # List cron jobs
    lenlion cron status         # Check if cron scheduler is running
    lenlion doctor              # Check configuration and dependencies
    lenlion honcho setup                    # Configure Honcho AI memory integration
    lenlion honcho status                   # Show Honcho config and connection status
    lenlion honcho sessions                 # List directory → session name mappings
    lenlion honcho map <name>               # Map current directory to a session name
    lenlion honcho peer                     # Show peer names and dialectic settings
    lenlion honcho peer --user NAME         # Set user peer name
    lenlion honcho peer --ai NAME           # Set AI peer name
    lenlion honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    lenlion honcho mode                     # Show current memory mode
    lenlion honcho mode [hybrid|honcho|local]  # Set memory mode
    lenlion honcho tokens                   # Show token budget settings
    lenlion honcho tokens --context N       # Set session.context() token cap
    lenlion honcho tokens --dialectic N     # Set dialectic result char cap
    lenlion honcho identity                 # Show AI peer identity representation
    lenlion honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    lenlion honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    lenlion version             Show version
    lenlion update              Update to latest version
    lenlion uninstall           Uninstall Hermes Agent
    lenlion acp                 Run as an ACP server for editor integration
    lenlion sessions browse     Interactive session picker with search

    lenlion claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``lenlion update``
...
# this guard, lenlion crashes on import and the user can't run
# ``lenlion update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``lenlion tools`` or
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`lenlion --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``lenlion --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (lenlion tools, lenlion setup, lenlion model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'lenlion {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any lenlion module import.
#
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `lenlion mcp add ... --args <command argv>`.
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo lenlion -p <name>` against the invoking user's home.
    ...
    # after the subcommand (`lenlion chat -p coder`), so keep scanning broadly.
    ...
    # /opt/data/profiles/coder).  If HERMES_HOME points to the lenlion root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `lenlion profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # 2. If no flag, check active_profile in the lenlion root.
    #
    ...
    # reserved ``gateway-default`` slot runs bare ``lenlion gateway run`` to mean
    ...
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent lenlion from starting
            print(
...
# Initialize centralized file logging early — all `lenlion` subcommands
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo lenlion {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo lenlion {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  lenlion --tui --resume {target}")
    if title:
        print(f'  lenlion --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @lenlion/ink is missing or node_modules is behind package-lock.json.
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        # Keep the local @lenlion/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @lenlion/ink resolves through
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `lenlion --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `lenlion update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``lenlion kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``lenlion kanban boards switch`` from another session can flip the file
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'lenlion sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'lenlion doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'lenlion setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `lenlion gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: lenlion gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `lenlion gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  lenlion gateway")
        ...
        print()
        print("  Or install as a service: lenlion gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'lenlion whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``lenlion whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``lenlion model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'lenlion model' for "
                "available providers, or run 'lenlion doctor' to diagnose config "
                "issues."
            )
...
# `lenlion model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `lenlion model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``lenlion model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
# the catalog eagerly costs ~55ms on every `lenlion` invocation — including
# fast paths like `lenlion --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``lenlion setup`` / ``lenlion model``.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `lenlion setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_slack(args):
    ...
    Dispatches ``lenlion slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: lenlion slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `lenlion slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_security(args):
    """Dispatch `lenlion security <subcmd>`."""
...
# Critical files that every ``lenlion`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``lenlion update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``lenlion`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``lenlion update --gateway`` so interactive prompts (stash restore,
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``lenlion`` launcher present but ``hermes_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the lenlion venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — lenlion venv python3, no nix-shell overhead
    ...
    # a lenlion venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``lenlion update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``lenlion web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (lenlion web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``lenlion update`` can unconditionally call ``lenlion desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``lenlion desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``lenlion update`` that pulled new source but hasn't built yet).
    """
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  lenlion gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> lenlion desktop --force-build")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``lenlion dashboard`` processes other than ourselves.

    ``lenlion dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``lenlion update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``lenlion dashboard`` as
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `lenlion update`.
    ...
    print("  Preview now:  lenlion curator run --dry-run")
    print("  Pause it:     lenlion curator pause")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``lenlion update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``lenlion update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: lenlion curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``lenlion dashboard`` processes.

    Called at the end of ``lenlion update`` (default ``reason``) and also
    from ``lenlion dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    lenlion dashboard --port <port>")
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `lenlion update "
            f"--branch {branch}`, or update against main with `lenlion update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: lenlion skills list-modified  "
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes lenlion completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `lenlion update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``lenlion update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``lenlion update`` mutates the
...
# survives and the next ``lenlion`` launch finishes the install instead of
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``lenlion update`` left half-done.
    ...
    protocol stream (``lenlion acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `lenlion update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``lenlion update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    so during ``lenlion update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``lenlion update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``lenlion update``
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Hermes Desktop, exit any open `lenlion` REPLs, and")
    lines.append("  stop the gateway (`lenlion gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `lenlion update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``lenlion update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    the next lenlion invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hermes Desktop, exit other `lenlion` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `lenlion update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    is left missing and ``lenlion`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every lenlion invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``lenlion update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `lenlion update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``lenlion update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``lenlion update``, not the venv we just wrote
    ...
    # live launcher, which would leave ``lenlion`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `lenlion update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `lenlion update --force` after closing other lenlion processes."
        )
        return
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `lenlion update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``lenlion update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``lenlion update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``lenlion update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``lenlion update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``lenlion update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``lenlion update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``lenlion update`` without requiring a reinstall.  The
    ...
    ``lenlion`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/lenlion.
    ...
    # /usr/local/bin/lenlion, code at /usr/local/lib/hermes-agent).
    fhs_link = Path("/usr/local/bin/lenlion")
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``lenlion update``
    ...
    print(f"  Restore:  lenlion import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    if unmapped_pids:
        ...
        print("    Restart manually after update: lenlion gateway run")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``lenlion update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `lenlion update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    print("✓ Update complete! Restart lenlion to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``lenlion update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``lenlion
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``lenlion update`` again later once a fix lands.")
                    else:
        ...
        # the marker survives and the next ``lenlion`` launch finishes the
        ...
        # build.  ``lenlion desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``lenlion desktop`` shouldn't be forced into a full
        # Electron build by ``lenlion update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `lenlion desktop` to retry)")
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'lenlion config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: lenlion config migrate")
            else:
                print()
                print("Skipped. Run 'lenlion config migrate' later to configure.")
        else:
        ...
        # check `lenlion curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``lenlion update`` gives users a predictable
        ...
        # When running as ``lenlion update --gateway`` (spawned by the gateway's
        ...
        try:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `lenlion update` restart it automatically, allow\n"
                            ...
                            # path in `lenlion gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: lenlion gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: lenlion gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: lenlion -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `lenlion update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    lenlion gateway migrate-legacy")
                print()
        ...
        print("  lenlion model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``lenlion -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `lenlion profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → lenlion -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  lenlion profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     lenlion -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `lenlion profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → lenlion -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  lenlion -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      lenlion -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `lenlion profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"lenlion -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
...
def _report_dashboard_status() -> int:
    """Print ``lenlion dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``lenlion dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No lenlion dashboard processes running.")
        return 0

    print(f"{len(pids)} lenlion dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No lenlion dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `lenlion update`.
...
# so that in ``lenlion -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``lenlion -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``lenlion`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `lenlion --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for lenlion CLI."""
    ...
    # ``lenlion update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``lenlion update``
    ...
    # ``lenlion skills install update``) merely defers recovery one launch;
    # under-matching (missing ``lenlion -p work update``) would race a recovery
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `lenlion model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `lenlion whatsapp` which sets up "
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        "repo lenlion uses to snapshot working directories before "
    ...
    # subcommand — ``lenlion --help``, ``lenlion version``, ``lenlion logs``,
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `lenlion computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `lenlion tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: lenlion computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: lenlion computer-use install")
            return
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch lenlion --resume <id> by replacing the current process
    ...
    # run `lenlion desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``lenlion -c Pokemon Agent Dev`` → ``lenlion -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'lenlion -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (lenlion hooks list, cron
