...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The dashboard source lives under ``web/``, and the Vite build outputs to
    ``hermes_cli/web_dist/`` (per vite.config.ts outDir).
...
def _update_node_dependencies() -> None:
    ...
    # Step 2: install the web workspace.
    ws_args = [*extra_args, "--workspace", "web"]
...
def cmd_dashboard(args):
    ...
    # Vue chat UI connects over /api/ws (tui_gateway JSON-RPC).
    start_server(
        host=args.host,
        port=args.port,
