...
* ``lenlion profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``lenlion skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``lenlion profile``, not a parallel tree):

    lenlion profile install <source> [--name N] [--alias] [--force] [--yes]
    lenlion profile update  <name>  [--force-config] [--yes]
    lenlion profile info    <name>
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``lenlion profile update`` can
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `lenlion profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`lenlion profile install <source> --name {canon} --force`."
        )
