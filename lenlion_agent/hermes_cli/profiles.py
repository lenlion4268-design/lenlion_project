...
Usage::
    ...
    lenlion profile create coder          # fresh profile + bundled skills
    lenlion profile create coder --clone  # also copy config, .env, SOUL.md, skills
    lenlion profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    lenlion -p coder chat                 # or via flag
    lenlion profile use coder             # set as sticky default
    lenlion profile delete coder          # remove profile + alias + service
"""
...
#   backups             — `lenlion backup` archives
...
# Marker file written by `lenlion profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `lenlion
...
# `lenlion skills install` or drop SKILL.md files into the profile's skills/.
...
_RESERVED_NAMES = frozenset({
    "hermes", CLI_COMMAND, "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the lenlion root, NOT to the current HERMES_HOME
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``lenlion``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``lenlion`` profile inside ``~/.hermes/``) or get refused
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, lenlion subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a lenlion subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            cli_exe = shutil.which(CLI_COMMAND) or CLI_COMMAND
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `lenlion doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``lenlion -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``lenlion profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``lenlion profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``lenlion update`` skips re-seeding this profile's
    ...
    # Keys pages, `lenlion -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `lenlion update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`lenlion profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `lenlion profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `lenlion -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``lenlion profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            ...
            "To remove everything, use: lenlion uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: lenlion profile create {canon}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: lenlion profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: lenlion profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any lenlion modules
