...
process replacement so that ``lenlion sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``lenlion`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the lenlion entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("lenlion")`` on PATH.
    ...
    # PATH lookup
    path_bin = shutil.which(CLI_COMMAND)
    if path_bin:
        return path_bin
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh lenlion invocation.
    ...
    relaunch contract wants: "run lenlion again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "lenlion exited, then
    new lenlion started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``lenlion`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nHermes relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run lenlion.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
