...
``lenlion gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``lenlion gateway start/stop/restart``).
    """
    ...
    # `lenlion gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged lenlion
    ...
    container drops to lenlion via ``s6-setuidgid``, that silent failure
...
# active (set via the ``lenlion -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `lenlion profile create`
...
# UID/GID of the in-image ``lenlion`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the lenlion user.
    ...
    (the ``lenlion`` user — UID 10000 — running every Hermes runtime
    ...
    is dispatched as the lenlion user.
    ...
    with lenlion ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           lenlion:lenlion, 0755 (parent must already exist)
    ``svc_dir/event/``                     lenlion:lenlion, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 lenlion:lenlion, 0755
    ``svc_dir/supervise/event/``           lenlion:lenlion, 03730
    ``svc_dir/supervise/control``          lenlion:lenlion, 0660    (FIFO)
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the lenlion user already — directory is hermes-
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``lenlion -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`lenlion profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the lenlion user and exec's
             ``lenlion -p <profile> gateway run`` (or just ``lenlion
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``lenlion gateway
        ...
        # (a stray ``lenlion gateway run`` from a shell, an agent action, or
        ...
        if profile == "default":
            gateway_cmd = "lenlion gateway run --replace"
        else:
            gateway_cmd = f"lenlion -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid lenlion {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``lenlion logs`` reads and what
        ...
        return (
            ...
            # later then runs its log service as lenlion and crash-loops on
            ...
            f'chown lenlion:lenlion "$HERMES_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R lenlion:lenlion "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid lenlion s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``lenlion gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``lenlion -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with lenlion ownership
            ...
            # the lenlion user) won't hit EACCES on root-owned 0700
