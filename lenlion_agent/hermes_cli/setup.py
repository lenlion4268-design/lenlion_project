...
# display_hermes_home imported lazily at call sites (stale-module safety during lenlion update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  lenlion config set model.provider custom")
    print_info("  lenlion config set model.base_url http://localhost:8080/v1")
    print_info("  lenlion config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'lenlion setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'lenlion setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'lenlion setup' to configure"))
    ...
    # Video generation — opt-in via `lenlion tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'lenlion setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'lenlion setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'lenlion setup terminal'"))
    ...
    # Spotify (OAuth via lenlion auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'lenlion setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('lenlion setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('lenlion setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('lenlion setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('lenlion setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('lenlion setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('lenlion config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('lenlion config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('lenlion config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('lenlion gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('lenlion doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `lenlion tools` and `lenlion setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``lenlion model``)
    ...
    provider added to ``lenlion model`` is automatically available here.
    ...
    # Delegate to the shared lenlion model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: lenlion model")
    ...
    # on demand via `lenlion auth add`, `lenlion setup` vision, and
    # `lenlion setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``lenlion model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via lenlion setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'lenlion setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `lenlion setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `lenlion setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `lenlion setup agent` later to customize.")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `lenlion update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        print_info(
            "   Re-run `lenlion slack manifest --write` anytime to refresh after "
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  lenlion config edit")
    print_info("   Open config in your editor:  lenlion config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'lenlion setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     lenlion config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo lenlion gateway install --system")
                print_info("  Or run in foreground:  lenlion gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   lenlion gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   lenlion gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `lenlion setup tools` and `lenlion tools` use the same flow:
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: lenlion claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: lenlion claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use lenlion claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``lenlion setup --portal`` and ``lenlion portal``. This is the
    ...
    ``lenlion setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``lenlion model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``lenlion portal`` always offers a model picker,
    ...
    # setup calls, so `lenlion portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `lenlion portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `lenlion portal`: %s", exc)
        print()
    ...
    print_info("  Run `lenlion portal info` to inspect routing.")
    print_info("  Run `lenlion` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      lenlion setup           — full or quick (auto-detected)
      lenlion setup model     — just model/provider
      lenlion setup tts       — just text-to-speech
      lenlion setup terminal  — just terminal backend
      lenlion setup gateway   — just messaging platforms
      lenlion setup tools     — just tool configuration
      lenlion setup agent     — just agent settings
    """
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'lenlion setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'lenlion config edit'")
    ...
    # Tune later with `lenlion setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``lenlion setup <section>``
    or switch providers with ``lenlion model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'lenlion setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    lenlion setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  lenlion setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'lenlion setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'lenlion setup gateway'.")
