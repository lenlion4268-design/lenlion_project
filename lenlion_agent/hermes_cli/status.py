"""
Status command for lenlion CLI.
...
from hermes_constants import CLI_COMMAND, OPENROUTER_MODELS_URL
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``lenlion config``'s
...
def show_status(args):
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: lenlion portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: lenlion model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: lenlion auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: lenlion auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: lenlion model)"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   lenlion gateway")
    ...
    print(color(f"  Run '{CLI_COMMAND} doctor' for detailed diagnostics", Colors.DIM))
    print(color(f"  Run '{CLI_COMMAND} setup' to configure", Colors.DIM))
    print()
