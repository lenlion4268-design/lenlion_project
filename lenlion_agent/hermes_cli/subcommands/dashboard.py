"""``lenlion dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    # for `lenlion dashboard` cmdlines and SIGTERM them directly — the same
    # path `lenlion update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running lenlion dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running lenlion dashboard processes and exit",
    )
    # Backward-compat shim: older Hermes desktop app shells (<= 0.15.x) spawn the
    # backend as `lenlion dashboard --no-open --tui --host ... --port ...`. The
    ...
    # `lenlion dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.hermes/.env.
    # Nested subparser so bare `lenlion dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "how to engage the login gate. Requires being logged in (lenlion setup)."
        ),
    )
