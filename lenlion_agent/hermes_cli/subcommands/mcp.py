...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'lenlion mcp add' to connect to a new server, or\n"
            "'lenlion mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    # flag is omitted, causing `lenlion mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `lenlion mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `lenlion mcp install n8n`)",
    )
