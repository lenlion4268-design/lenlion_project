...
`lenlion tools` and `lenlion setup tools` both enter this module.
...
# who want it opt in via `lenlion tools` → Video Generation, which walks
...
# `lenlion tools` → X (Twitter) Search setup walks users through credential
...
# Platform-scoped toolsets: only appear in the `lenlion tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``lenlion tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``lenlion tools`` checklist actually offers
    for ``platform``.
    ...
    so ``lenlion tools`` never claims to add or remove a toolset the user was
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``lenlion update`` and
      by ``lenlion computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `lenlion update` calls this for every
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Run the full `lenlion auth spotify` flow — if the user has no
        ...
        try:
            ...
            _print_info("    Run manually: lenlion auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `lenlion auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: lenlion auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: lenlion plugins enable observability/langfuse")
        _print_info("    Restart Hermes for tracing to take effect.")
        _print_info("    Verify: lenlion plugins list")
        ...
        try:
            ...
            _print_info("    Run later: lenlion auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `lenlion auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: lenlion auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: lenlion auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``lenlion tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``lenlion tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: lenlion tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[hermes-cli, spotify]`` after enabling Spotify via ``lenlion
        ...
        # through ``lenlion tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `lenlion tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `lenlion tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `lenlion tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by lenlion tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `lenlion tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `lenlion tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `lenlion model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `lenlion tools` and `lenlion setup tools`.
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, lenlion mcp
