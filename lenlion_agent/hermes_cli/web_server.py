...
try:
    ...
    # running `lenlion dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``lenlion gateway run`` — but the
    desktop app spawns a ``lenlion dashboard`` backend, not a gateway, so a cron
...
class ModelAssignment(BaseModel):
    ...
    # ``lenlion model`` custom flow collects. Honored only on the main slot for
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``lenlion update``.
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``lenlion status`` and the
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the lenlion home root).
    """
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``lenlion <subcommand>`` detached and record the Popen handle.
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join([CLI_COMMAND, *_gateway_subcommand(profile, verb)])


def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``lenlion gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``lenlion gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``lenlion gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``lenlion update`` in the background."""
    ...
    try:
        ...
        _log.exception("Failed to spawn lenlion update")
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/hermes/update`` actually runs ``lenlion update``.
...
def get_model_options(profile: Optional[str] = None):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `lenlion model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `lenlion model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``lenlion model`` custom flow
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in lenlion gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in lenlion gateway setup",
...
# the canonical ``lenlion auth add <provider>`` command so the dashboard
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": f"{CLI_COMMAND} auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": f"{CLI_COMMAND} auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": f"{CLI_COMMAND} auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": f"{CLI_COMMAND} auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": f"{CLI_COMMAND} auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": f"{CLI_COMMAND} auth add anthropic",
...
# Guarded so lenlion web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``lenlion auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
# the tokens exactly like `lenlion auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `lenlion auth add xai-oauth`'s credential-pool insert.
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``lenlion auth add minimax-oauth``.
    """
    from hermes_cli.auth import (
        _minimax_poll_token,
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `lenlion sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `lenlion sessions prune`)."""
...
# servers managed here show up under `lenlion mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `lenlion mcp catalog` / `lenlion mcp install` read.  ``profile`` scopes
...
# spawn the real `lenlion gateway <verb>` so behaviour matches the CLI exactly.
...
# `lenlion memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `lenlion memory setup` to configure a new one.",
            )
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `lenlion import`. The spawned action runs with
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``lenlion`` subprocess, and
...
# Human-readable labels for each hub source id (matches `lenlion skills search`
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`lenlion -p <name> skills install`)
...
def _profile_setup_command(name: str) -> str:
    ...
    return "lenlion setup" if name == "default" else f"{name} setup"
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``lenlion profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``lenlion profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``lenlion profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``lenlion tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``lenlion tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``lenlion tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``lenlion tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``lenlion tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``lenlion -p <profile> tools post-setup``.
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"lenlion auth {name}"
                        break
            except Exception:
                pass
