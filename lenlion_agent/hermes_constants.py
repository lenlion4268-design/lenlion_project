...
# Installed CLI command name (``pip install -e .`` console script).
CLI_COMMAND = "lenlion"

# Env vars checked for an explicit CLI binary override (first match wins).
CLI_BIN_ENV_VARS: tuple[str, ...] = ("LENLION_BIN", "HERMES_BIN")


def get_cli_command() -> str:
    """Return the CLI command name users type (``lenlion``)."""
    return CLI_COMMAND


def get_cli_bin_from_env() -> str:
    """Return explicit CLI binary from ``LENLION_BIN`` / ``HERMES_BIN``, if set."""
    for key in CLI_BIN_ENV_VARS:
...
def get_default_hermes_root() -> Path:
    ...
    (``~/.hermes`` on POSIX, ``%LOCALAPPDATA%\\lenlion`` on native Windows).
