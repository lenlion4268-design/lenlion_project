...
lenlion meet install                 # pip deps + Chromium (transcribe only)
lenlion meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
lenlion meet auth                    # optional; skips guest-lobby wait
lenlion meet setup                   # preflight checks
```

`lenlion meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
lenlion plugins enable google_meet
lenlion meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
lenlion meet node approve my-mac ws://<mac-ip>:18789 <token>
lenlion meet node ping my-mac                   # confirm reachable
```

Run `lenlion meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `lenlion meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `lenlion meet node approve`.
