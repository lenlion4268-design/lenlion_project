"""`lenlion meet node ...` subcommand tree.

Wired into the existing ``lenlion meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``lenlion meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``lenlion meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             lenlion meet node approve <name> ws://<host>:{args.port} {token}")
        try:
