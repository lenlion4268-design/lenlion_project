...
```bash
lenlion memory setup honcho   # configure Honcho directly (works on a fresh install)
lenlion memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
lenlion config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hermes/.env
```

> `lenlion honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `lenlion memory setup honcho`.
...
Host key is derived from the active Hermes profile: `lenlion` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `lenlion honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 7 | `global` strategy | Workspace name (`lenlion`) |
...
- **`per-directory`** — basename of `$PWD`. Opening lenlion in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Hermes session ID (timestamp + hex). Every `lenlion` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
...
Both profiles see the same user (`yourname`) in the same shared environment (`lenlion`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Hermes profile: `lenlion` (default) or `hermes_<profile>` (e.g. `lenlion -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `lenlion memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `lenlion honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `lenlion memory setup`) |
| `lenlion honcho status` | Show resolved config for active profile |
| `lenlion honcho enable` / `disable` | Toggle Honcho for active profile |
| `lenlion honcho mode <mode>` | Change recall or observation mode |
| `lenlion honcho peer --user <name>` | Update user peer name |
| `lenlion honcho peer --ai <name>` | Update AI peer name |
| `lenlion honcho tokens --context <N>` | Set context token budget |
| `lenlion honcho tokens --dialectic <N>` | Set dialectic max chars |
| `lenlion honcho map <name>` | Map current directory to a session name |
| `lenlion honcho sync` | Create host blocks for all Hermes profiles |
