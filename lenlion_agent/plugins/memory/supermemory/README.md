...
```bash
lenlion memory setup    # select "supermemory"
```

Or manually:

```bash
lenlion config set memory.provider supermemory
...
|-----|---------|-------------|
| `container_tag` | `lenlion` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hermes-{identity}` → `hermes-coder`). |
...
All Supermemory API calls send `x-sm-source: lenlion`, and document writes stamp
`metadata.sm_source: lenlion`. This is a **functional routing key, not telemetry**:
