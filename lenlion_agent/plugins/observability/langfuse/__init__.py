...
load when listed in ``plugins.enabled`` (via ``lenlion plugins enable
observability/langfuse`` or ``lenlion tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``lenlion tools`` or ~/.hermes/.env):
