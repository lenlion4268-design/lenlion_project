...
```bash
lenlion plugins enable observability/nemo_relay
```
...
`lenlion plugins enable ...` and the later `lenlion chat ...` run. If unset,
...
lenlion plugins enable observability/nemo_relay
lenlion chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `lenlion` command you run is built from the
...
uv run lenlion plugins enable observability/nemo_relay
uv run lenlion chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/lenlion_agent-*.whl
