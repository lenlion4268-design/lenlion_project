...
# ``model_tools`` import time, so ``lenlion status``, ``lenlion chat``, etc.
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a lenlion MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a lenlion MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``lenlion setup``.
    ...
    print_info("Restart the gateway: lenlion gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``lenlion cron`` running as a
    separate process from ``lenlion gateway``).  Without this hook,
