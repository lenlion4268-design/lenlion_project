...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `lenlion photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `lenlion setup` / status.
        return False
    return True
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `lenlion photon setup`)"
            )
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `lenlion photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: lenlion photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `lenlion gateway setup` alongside every other
    ...
    # Register CLI subcommands — `lenlion photon ...`
