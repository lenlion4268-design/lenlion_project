"""
``lenlion photon ...`` CLI subcommands — registered by the plugin via
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `lenlion photon ...` subcommands."""
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``lenlion photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    print("  Start the gateway:  lenlion gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `lenlion photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`lenlion photon telemetry on|off`)")
    return 0
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `lenlion photon telemetry on` / `lenlion photon telemetry off`.")
        return 0
    try:
    ...
    print("  Restart the gateway for the sidecar to pick it up:  lenlion gateway restart")
    return 0
...
#
# `lenlion gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``lenlion photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `lenlion gateway setup` wizard."""
