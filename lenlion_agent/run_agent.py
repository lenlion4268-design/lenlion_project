...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``lenlion -c`` / ``--resume`` (relaunch
...
class AIAgent:
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``lenlion chat -q``. In that mode,
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        if _dev:
            ...
            # `lenlion logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``lenlion auth add xai-oauth``) whose tokens belong to
