...
#   code at /usr/local/lib/hermes-agent, command at /usr/local/bin/lenlion,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   'lenlion update' runs never inject bundled skills either"
            ...
            echo "  /usr/local/bin/lenlion (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/lenlion-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs lenlion setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `lenlion update`
...
# Decide where the repo checkout + venv live, and where the `lenlion` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/lenlion-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/lenlion-agent
...
#                              $HERMES_HOME/lenlion-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/lenlion-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/lenlion-agent/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/lenlion-agent"
        ...
        # /usr/local/bin/lenlion wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/lenlion"
...
install_uv() {
    ...
    # place, so install.sh and `lenlion update` stay in sync.
...
install_node() {
    ...
    # the lenlion command uses (get_command_link_dir): /usr/local/bin for root
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `lenlion update` path
                # (#4735).
...
setup_path() {
    log_info "Setting up lenlion command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        HERMES_BIN="$(which lenlion 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "lenlion not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "lenlion entry point not found at $HERMES_BIN"
    ...
    # Create a user-facing shim for the lenlion command.
...
EOF
    ...
    log_success "Installed lenlion launcher → $command_link_display_dir/hermes"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "lenlion command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "lenlion not on PATH in non-login shells (common on RHEL-family)"
    ...
    # Export for current session so lenlion works immediately
    export PATH="$command_link_dir:$PATH"
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `lenlion update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'lenlion update'." \
        ...
        log_info "  Future 'lenlion update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (lenlion --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `lenlion update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'lenlion setup' after install."
        return 0
    fi
    ...
    # Run lenlion setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'lenlion whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'lenlion whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'lenlion gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: lenlion gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: lenlion gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
...
print_success() {
    ...
    echo -e "   ${GREEN}lenlion setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}lenlion config${NC}       View/edit configuration"
    echo -e "   ${GREEN}lenlion config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}lenlion gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}lenlion update${NC}       Update to latest version"
    echo ""
...
postinstall_mode() {
    ...
    HERMES_CMD="$(command -v lenlion 2>/dev/null || echo "")"
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running lenlion setup..."
        "$HERMES_CMD" setup
    else
        log_warn "lenlion command not found on PATH"
...
# _purge_electron_build_cache() used by `lenlion desktop`; install.sh was the only
...
install_desktop() {
    ...
    #         retry (matches install.ps1 / `lenlion desktop`),
    ...
    # checkout stays clean for the next `lenlion update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        complete)
            ...
            # 'lenlion update' on this host install. See detect_install_method().
...
main() {
    ...
    # stamp and wrongly blocks 'lenlion update' on this host install.
