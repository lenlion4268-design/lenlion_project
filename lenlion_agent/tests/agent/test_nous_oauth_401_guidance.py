...
to a generic "Your API key was rejected... run lenlion setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "lenlion setup".
    assert "lenlion portal" in source
