...
@pytest.fixture()
def mock_config():
    """Return a minimal lenlion config dict suitable for unit tests."""
    return {
...
def _live_system_guard(request, monkeypatch):
    ...
    targeting lenlion/python patterns are also blocked.
    """
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting lenlion/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `lenlion update` (or the
        ...
        if "update" in low and (
            # lenlion update / lenlion update --gateway / setsid bash -c ... lenlion update
            ...
            raise RuntimeError(
                ...
                "`lenlion update` against the real checkout, fetching "
