...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        # `lenlion update --gateway` against the CI checkout. The managed-install
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when lenlion is not on PATH and hermes_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "lenlion update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m hermes_cli.main when lenlion not on PATH."""
        runner = _make_runner()
        event = _make_event()
