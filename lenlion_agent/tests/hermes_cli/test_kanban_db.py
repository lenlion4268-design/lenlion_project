...
# spawned with `lenlion -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Worker's perspective (profile activated by `lenlion -p coder`).
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'lenlion kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
#
# Workers spawned by the dispatcher must use a `lenlion` invocation that does
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    """When `lenlion` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import hermes_cli.kanban_db as kb

    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/lenlion")
    argv = kb._resolve_hermes_argv()
    assert argv == ["/usr/local/bin/lenlion"]
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    Pins the correct module name (NOT `lenlion` — there is no top-level
    `lenlion` package). Regression for #23198: the original PR shipped
    `python -m lenlion` which fails with `No module named lenlion` on every
    invocation.
    """
    import shutil
    import sys
