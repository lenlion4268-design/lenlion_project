...
- Bare `lenlion setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `lenlion setup --quick` runs the narrower "fill in missing items" flow.
- `lenlion setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `lenlion setup` on an existing install = full reconfigure wizard."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`lenlion setup --reconfigure` behaves the same as bare `lenlion setup`."""
