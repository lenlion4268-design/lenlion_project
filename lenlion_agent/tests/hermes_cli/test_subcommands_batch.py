...
# ── deprecated `lenlion login` fails gracefully, not with argparse error ────
#
# `lenlion login` is a removed command; its handler (`login_command` in
# `hermes_cli/auth.py`) prints a deprecation notice pointing at `lenlion auth` /
# `lenlion model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `lenlion --help`.
