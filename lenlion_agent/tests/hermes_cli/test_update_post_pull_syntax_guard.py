"""Tests for the post-pull syntax guard in ``lenlion update``.
...
becomes unbootable — every ``lenlion`` invocation imports those files at
...
markers in ``hermes_cli/config.py``; users who ran ``lenlion update`` in
the 7-minute window before #28458 landed could not run any ``lenlion``
command afterward.
"""
...
# release; if a future ``lenlion update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"lenlion update would brick users. Error: {error}"
    )
