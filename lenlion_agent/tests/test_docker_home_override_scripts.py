...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid lenlion lenlion dashboard" in text
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'chown -R lenlion:lenlion "$HERMES_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$HERMES_HOME/cron" ]; then' in text
    assert 'chown -R lenlion:lenlion "$HERMES_HOME/cron" 2>/dev/null || true' in text
