...
(lenlion, hermes-agent, hermes-acp, gateway, batch_runner, cli.py).  It
...
class TestEntryPointsImportBootstrap:
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``lenlion update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hermes_bootstrap`` as a top-level module) leaves lenlion
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lenlion start even when hermes_bootstrap hasn't been
