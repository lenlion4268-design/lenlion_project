...
def test_dockerfile_makes_opt_hermes_root_owned_and_non_writable() -> None:
    ...
    assert "COPY --chown=lenlion:lenlion . ." not in text
    assert "COPY . ." in text
...
def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+lenlion:hermes\s+/opt/hermes/\.venv",
        r"chown\s+-R\s+lenlion:hermes\s+/opt/hermes/ui-tui",
        r"chown\s+-R\s+lenlion:hermes\s+/opt/hermes/gateway",
        r"chown\s+-R\s+lenlion:hermes\s+/opt/hermes/node_modules",
    )
