...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "lenlion skills reset new-skill" in captured
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `lenlion profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.hermes) rely on so bundled
    skills are never seeded at install time NOR re-injected by `lenlion update`.
    """
...
class TestOptOutToggleAndRemove:
    """`lenlion skills opt-out/opt-in` core: marker toggle + safe removal."""
