...
/opt/hermes must remain root-owned/non-writable by the runtime lenlion user so
...
def test_stage2_does_not_chown_install_tree_to_hermes(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to lenlion; "
