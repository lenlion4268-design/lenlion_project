...
by `docker exec <container> lenlion …` (root unless `-u` is passed) they land
root-owned and the unprivileged lenlion runtime then hits PermissionError on next
startup.
...
def _toplevel_chown_loop(text: str) -> str:
    """Extract the `for f in … chown lenlion:lenlion "$HERMES_HOME/$f" … done`
    ...
    block = m.group(1)
    assert 'chown lenlion:lenlion "$HERMES_HOME/$f"' in block, (
