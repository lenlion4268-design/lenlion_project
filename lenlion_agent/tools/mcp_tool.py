...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `lenlion mcp login {server_name}` (or delete the tokens "
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``lenlion tools`` interactive configuration — connects to each
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple lenlion
