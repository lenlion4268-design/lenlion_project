...
provider** — turn on a backend by enabling a plugin (``lenlion plugins
enable video_gen/<name>``) and selecting it in ``lenlion tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `lenlion tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `lenlion plugins list` to see "
            f"installed video gen backends, or `lenlion tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `lenlion tools` → "
...
# `lenlion tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`lenlion tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `lenlion tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
