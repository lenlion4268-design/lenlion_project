import type { RunExternalProcess } from '@lenlion/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`lenlion ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching lenlion: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`lenlion ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
